...
public class GiftCertificateDtoValidation {
    ...
    public static boolean chechCertificateDtoFormat(GiftCertificateDto dto) {
        ...
        if (dto.getName() == null || dto.getName().isBlank() || dto.getName().length() < 3 || dto.getName().length() >32) {
            throw new GiftCertificateDtoValidationException(INVALID_DTO_NAME);
        } else if (dto.getDescription() == null || dto.getDescription().isBlank() || dto.getDescription().length() < 5 || dto.getDescription().length() > 100) {
